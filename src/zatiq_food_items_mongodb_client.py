...
class ZatiqFoodItemsMongoDBClient(object):
    def add_food_item(self, image, image_aspect_ratio, overview, item_name, api_token, meal_type, tags, item_price, bouffet_item, meat, seafood):
        if self.check_valid_api_token(api_token) == True:
            restaurant = self.get_restaurant_id_by_api_token(api_token)
            try:
                add_food_item = Zatiq_Food_Items(restaurant_id=restaurant, item_name=item_name, image=image, image_aspect_ratio=image_aspect_ratio, overview=overview, meal_type=meal_type, is_beverage=tags['is_beverage'], item_price=item_price, bouffet_item=bouffet_item,
